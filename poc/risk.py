...
# --- Маскирование PII --------------------------------------------------------
_PII_PATTERNS = [
    # Номер карты: 13-19 цифр, между которыми допустимы пробел или дефис.
    # Разделитель ищется ТОЛЬКО между цифрами: шаблон вида (?:\d[ -]?){13,19}
    # захватывал бы и пробел после последней цифры, склеивая слова
    # («карта [CARD]рядом») и портя текст в аудит-логе и в промпте.
    ("CARD", re.compile(r"\b\d(?:[ -]?\d){12,18}\b")),
    # Домен из нескольких уровней: без (?:[\w-]+\.)+ адрес вида
    # a@example.co.uk маскировался частично и оставлял хвост «.uk».
    ("EMAIL", re.compile(r"\b[\w.+-]+@(?:[\w-]+\.)+[a-zA-Zа-яА-Я]{2,}\b")),
    # Телефон: сначала российский формат, затем общий международный
    # (плюс, код страны и 9-17 символов из цифр и разделителей).
    # Без второй ветки номер вида +1 415 555 2671 уходил бы во внешний API
    # незамаскированным — это нарушение требования не передавать PII.
    ("PHONE", re.compile(
        r"(?:\+7|\b8)[\s-]?\(?\d{3}\)?[\s-]?\d{3}[\s-]?\d{2}[\s-]?\d{2}"
        r"|\+\d{1,3}[\s\-()]?\d(?:[\s\-()]?\d){8,14}"
    )),
]
