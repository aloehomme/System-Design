...
УПРОЩЕНИЕ: холодный путь вызывается синхронно, сразу после горячего, внутри
того же процесса - разделение здесь логическое, а не физическое. В целевой
системе между ними стоит очередь (Kafka) и пул воркеров генерации,
идемпотентных по ticket_id.
"""
