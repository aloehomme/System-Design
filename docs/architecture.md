...
graph TB
    ...
    C1 & C2 & C3 & C4 --> GW --> PII --> EMB --> CLF --> RISK --> POL
...
Значения по этапам — **проектные оценки, а не замеры**: они задают целевой
бюджет, который проверяется нагрузочным тестом на этапе M2 (docs/product.md).
Замерена в этом проекте только латентность вызова LLM (4 240 мс) — то
единственное число, на котором держится решение о разделении путей.
...
**Измерения в PoC:** горячий путь без эмбеддинг-модели даёт p50 ≈ 0,13 мс
и p95 в пределах 0,2–0,4 мс на 200 прогонах (абсолютные значения зависят
от машины и её загрузки). Это подтверждает только то, что правила и policy
engine стоят пренебрежимо мало; основной бюджет в целевой системе съедают
эмбеддинги и сеть, которых в PoC нет.

---

## 5. Синхронные и асинхронные части

**Нагрузка, под которую проектируется система** (заданные вводные):
200 000 тикетов/сутки — это в среднем **2,3 тикета/с**, но при инциденте
10–20 тыс. тикетов за 10 минут дают пик **33 тикета/с, то есть 14× от среднего**.
Именно пик, а не среднее, определяет размер горячего пути: он должен держать
33 RPS с латентностью p95 ≤ 500 мс, иначе SLA первого ответа (15 минут)
нарушается ровно в тот момент, когда обращений больше всего.

| Свойство | Горячий путь | Холодный путь |
|---|---|---|
| Что делает | Классификация, риск, маршрутизация | Генерация ответа, отправка |
| Расчётная нагрузка | 2,3 RPS в среднем, 33 RPS на пике | То же, но пик сглаживается очередью |
...
Кэш ответов заслуживает отдельного пояснения: заметная часть тикетов в
безопасных категориях — это перефразировки одного и того же вопроса
(«где заказ»). Кэширование по паре (статья базы знаний + нормализованный
вопрос) снимает часть вызовов LLM. Насколько большую — на этих данных сказать
нельзя, поэтому в расчёте unit-экономики (docs/product.md) эффект кэша
**не учитывался вовсе**: это сознательно консервативная оценка, а доля
попаданий в кэш измеряется на этапе shadow.
...
| Выборочный контроль | Супервизор | Ежедневная проверка 500 случайных автоответов (≈1,1% от 44 тыс. в сутки, см. docs/monitoring.md) |
